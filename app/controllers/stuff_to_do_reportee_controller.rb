class StuffToDoReporteeController < ApplicationController
  ...
  def add
	...
	connection.execute("INSERT INTO `stuff_to_do_reportees` SET `reportee_id`=#{@user.id.to_i}, `user_id`=#{reportee_id.to_i}");
        if !StuffToDoReportee.new(:reportee => @user, :user => User.find(reportee_id)).save
...
#    end
  ...
  def delete
    ...
    connection = ActiveRecord::Base.connection
    connection.execute("DELETE FROM `stuff_to_do_reportees` WHERE `reportee_id`=#{@user.id.to_i} AND `user_id`=#{params[:manager_id].to_i}");
#    if !@user.nil? && !params[:reportee_id].nil?
#      StuffToDoReportee.where(:reportee_id => @user.id, :user_id => params[:reportee_id]).each do |manager|
#	connection.execute("DELETE FROM `stuff_to_do_reportees` WHERE `reportee_id`=#{@user.id.to_i}, `user_id`=#{manager.id.to_i}");
#        manager.destroy
#      end
#    end
...
#    end
  ...
  def get_user
    ...
    if params[:reportee_id]
      if User.current.admin? || (User.current.id.to_s == params[:reportee_id])
        @user = User.find(params[:reportee_id])
      else
        deny_access
      end
    else
